...
async function renderShelf() {
  ...
  if (shelfSort.on) list.sort((a, b) => shelfSort.by === 'date'
...
function beginCardDrag(x, y) {
  ...
  if (!grid) return;
  // при включённой сортировке ручной порядок не действует — перетаскивание книг
  // молча бы перезаписало ord по отсортированной сетке и испортило ручной порядок
...
async function renderCatShelf() {
  ...
  const cards = list.map(en => {
    ...
    const busy = catBusyKeys.has(en.key);
    const badge = dlId
      ? `<span class="cat-badge done" title="${esc(t('catDoneT'))}"><svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="m4.5 12.5 5 5 10-11"/></svg></span>`
      : busy
      ? `<span class="cat-badge busy" aria-hidden="true"><span class="cat-spin"></span></span>`
      : `<span class="cat-badge" title="${esc(t('catDlT'))}"><svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 4v12"/><path d="m6.5 11.5 5.5 5.5 5.5-5.5"/><path d="M5 20h14"/></svg></span>`;
    ...
    return `<div class="book-card cat-card" data-catbook="${i}">
      <button class="cover">${face}${badge}</button>
      <div class="book-meta">
...
function renderCatAudioShelf() {
  ...
  const cards = list.map(en => {
    ...
    return `<div class="ab-card cat-card" data-catab="${i}">
      <button class="ab-card-cover"><span>♪</span>${badge}</button>
...
function bindUI() {
  ...
  $('#tab-audio').addEventListener('click', e => {
    ...
    // аудио-каталог: добавить в библиотеку / открыть добавленную / дозагрузка / повтор
    if (activeCat && activeCat.kind === 'audio') {
      ...
      if (cb) {
        ...
        if (en) {
          const haveId = catBookIdOf(en);
          if (haveId) location.hash = '#/a/' + haveId;
          else catDownloadAudio(en);
        }
        return;
      }
  ...
  // Не отпустил и повёл — то же удержание переходит в перетаскивание карточки (ручной порядок).
  {
    ...
    addEventListener('touchstart', e => {
      ...
      const card = e.target.closest('.book-card, .ab-card');
      if (!card || card.classList.contains('cat-card')) return;   // карточки каталога не выбираются и не таскаются
    ...
    addEventListener('pointerdown', e => {
      ...
      const card = e.target.closest('.book-card, .ab-card');
      if (!card || card.classList.contains('cat-card')) return;
      const already = selMode && (card.classList.contains('ab-card') ? 'audio' : 'books') === selKind;
