...
const GENRES = ['Фэнтези', 'Фантастика', 'Роман', 'Детектив', 'Приключения',
  'Ужасы', 'Классика', 'Ранобэ', 'Манга', 'Научпоп', 'Поэзия', 'Драма', 'Другое'];
function mapGenre(raw) {
  const s = String(raw || '').toLowerCase();
  if (!s) return '';
  const has = (...w) => w.some(x => s.includes(x));
  // сначала жанр по СОДЕРЖАНИЮ — он важнее формата (ранобэ/манга — это формат, не жанр)
  if (has('fantasy', 'фэнтези', 'фентези', 'sf_fantasy', 'magic', 'магия', 'магии', 'волшебств',
  ...
  if (has('sci-fi', 'sci_fi', 'science fiction', 'фантаст', 'sf_', 'киберпанк', 'cyberpunk',
    'космос', 'звездолёт', 'галактик', 'робот', 'андроид', 'постапокал')) return 'Фантастика';
  if (has('detective', 'детектив', 'триллер', 'thriller', 'mystery', 'крими', 'расследов')) return 'Детектив';
  if (has('horror', 'ужас', 'мистик', 'хоррор')) return 'Ужасы';
  if (has('adventure', 'приключ', 'action', 'боевик')) return 'Приключения';
  if (has('classic', 'классик', 'prose_classic')) return 'Классика';
  if (has('science', 'научн', 'нон-фикшн', 'nonfiction', 'sci_', 'познавател', 'бизнес')) return 'Научпоп';
  if (has('poetry', 'поэз', 'стих', 'verse')) return 'Поэзия';
  if (has('drama', 'драма', 'пьеса', 'play')) return 'Драма';
  if (has('romance', 'любов', 'love', 'мелодрам')) return 'Роман';
  ...
  if (has('ранобэ', 'ранобе', 'ranobe', 'light novel', 'веб-новелл', 'web novel', 'litrpg', 'лит-рпг')) return 'Ранобэ';
