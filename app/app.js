...
const I18N = {
  ru: {
    ...
    grabNeedLoginQ: 'Этот раздел отдаёт содержимое только после входа в аккаунт. Войти сейчас?',
    grabLoginYes: 'Войти', grabLoginFail: 'Содержимое всё равно не отдают — вход не помог',
    grabLoginNoNat: 'Вход в аккаунт работает только в приложении на телефоне',
  ...
  en: {
    ...
    grabNeedLoginQ: 'This section only serves content to signed-in accounts. Sign in now?',
    grabLoginYes: 'Sign in', grabLoginFail: 'Still not served — signing in did not help',
    grabLoginNoNat: 'Signing in works only in the phone app',
...
async function grabMod() {
  if (!window.WebGrab) await loadLazyScript('grab.js?v=2');
  // токен аккаунта (если раньше входили) — с ним закрытые разделы отдают содержимое
  try { window.WebGrab.setToken(await kvGet('grabToken') || ''); } catch {}
  return window.WebGrab;
}
// ── вход в аккаунт сайта ──
// Страницу входа открываем В ОКНЕ ПРИЛОЖЕНИЯ (нативный мост): читать хранилище чужого браузера
// система не даёт — приложения изолированы, поэтому «войти в Chrome и подхватить оттуда» невозможно.
// Пользователь входит сам (в том числе через сторонний сервис), мы лишь забираем токен у себя.
// Возвращает токен или '' (закрыли окно / не получилось). Токен остаётся на устройстве.
function siteLogin(host) {
  const nat = window.AndroidSiteLogin;
  if (!nat || !nat.open) { showToast(t('grabLoginNoNat')); return Promise.resolve(''); }
  return new Promise(resolve => {
    let done = false;
    window.__siteLogin = async tok => {
      if (done) return; done = true;
      window.__siteLogin = null;
      tok = (tok || '').trim();
      if (tok) { try { await kvSet('grabToken', tok); } catch {} }
      resolve(tok);
    };
    try { nat.open('https://' + host + '/'); }
    catch { done = true; window.__siteLogin = null; resolve(''); }
  });
}
...
async function grabImport(url) {
  ...
  try {
    ...
    // Закрытый раздел выдаёт себя однозначно: список глав пришёл, а содержимое главы — нет.
    // Только в этом случае (а не «на всякий случай») предлагаем вход и повторяем уже с токеном.
    let first = await G.chapter(det, chs[0]).catch(() => null);
    if (!first) {
      hideToast();
      if (!await uiConfirm(t('grabNeedLoginQ'), { yes: t('grabLoginYes') })) return true;   // передумали — тихо выходим
      const tok = await siteLogin(det.host);
      if (tok) { G.setToken(tok); first = await G.chapter(det, chs[0]).catch(() => null); }
      if (!first) throw new Error(t('grabLoginFail'));
    }
